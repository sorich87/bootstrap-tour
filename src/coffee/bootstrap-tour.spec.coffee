describe 'Bootstrap Tour', ->
  ...
  #     afterSetState: -> true
  #     afterGetState: -> true
  #   expect(@tour.options.name).toBe 'test'
  #   expect(@tour.options.afterGetState).toBeTruthy
  #   expect(@tour.options.afterSetState).toBeTruthy

  # it 'should have `tour` as default name', ->
  #   @tour = new Tour
  #   expect(@tour.options.name).toBe 'tour'

  # it 'should accept an array of steps', ->
  #   @tour = new Tour
  #   expect(@tour.options.steps).toEqual [] # tour accepts an array of steps

  it '`_setState` should save state as localStorage item', ->
    @tour = new Tour

    @tour._setState('test', 'yes')
    expect(window.localStorage.getItem("#{@tour.options.name}_test")).toBe 'yes'
  ...
  it '`_removeState` should remove state localStorage item', ->
    ...
    expect(window.localStorage.getItem("#{@tour.options.name}_test")).toBeNull()
  ...
  it '`_removeState` should remove state internally if storage is false', ->
    ...
    @tour._setState('test', 'yes')
    @tour._removeState('test')
    expect(@tour._state['test']).toBeNull()
  ...
  it '`_getState` should get state localStorage items', ->
    ...
    window.localStorage.setItem("#{@tour.options.name}_test", null)
  ...
  it '`addStep` should add a step', ->
    ...
    @tour.addStep(step)
    expect(@tour.options.steps[0]).toBeTruthy
  ...
  it '`addSteps` should add multiple step', ->
    ...
    expect(@tour.options.steps[0]).toBeTruthy
    expect(@tour.options.steps[1]).toBeTruthy

  # it 'should set an unique id to each step', ->
  #   @tour = new Tour
  ...
  # it 'should execute `onStart` before showing the first step', ->
  #   test = 0
  #   @tour = new Tour onStart: -> test += 2

  #   @tour.addStep element: $('<div></div>').appendTo('body')
  #   @tour.start()
  #   expect(test).toBe 2

  # it 'should execute `onEnd` after hiding the last step', ->
  #   test = 0
  #   @tour = new Tour onEnd: -> test += 2
  ...
  # it 'should execute `onShow` before showing the step', ->
  #   test = 0
  #   @tour = new Tour()
  ...
  # it 'should execute `onShown` after showing the step', ->
  #   test = 0
  #   @tour = new Tour()
  ...
  # it 'should execute `onHide` before hiding the step', ->
  #   test = 0
  #   @tour = new Tour()
  ...
  # it 'should execute `onHidden` after hiding the step', ->
  #   test = 0
  #   @tour = new Tour()
  ...
  it '`step` should get a step', ->
    ...
    @tour.addStep(step)
    expect(@tour.step(0).options).toEqual step
  ...
  #   @tour.start()
  #   @tour.next()
  #   expect(@tour.step(0).element.data('bs.popover')).toBeUndefined() # tour hides current step
  #   expect(@tour.step(1).element.data('bs.popover').tip().filter(':visible').length).toBe 1 # tour shows next step
  ...
  it '`hideStep` should hide a step', ->
    ...
    expect(@tour.step(0).options.element.data('bs.popover')).toBeUndefined()
  ...
  it '`showStep` should set a step and show it', ->
    ...
    expect(@tour.step(1).options.element.data('bs.popover')).toBeDefined()
  ...
  it '`showStep` should not show anything when the step does not exist', ->
    ...
    @tour.addStep element: $('<div></div>').appendTo('body')
    @tour.showStep(1)
    expect(@tour.step(1).options.element.data('bs.popover')).toBeUndefined()
  ...
  it '`showStep` should execute template if it is a function', ->
    ...
    @tour.showStep(0)
    expect(@tour.step(0).options.element.data('bs.popover')).toBeDefined()
  ...
  #   expect(@tour.current).toBe 4 # tour sets current step if passed a value
  #   @tour._setState('current_step', 2)
  #   @tour.setCurrentStep()
  #   expect(@tour.current).toBe 2 # tour reads current step state if not passed a value
  ...
  #   @tour.addStep({element: $('<div></div>').appendTo('body')})
  #   @tour.goTo(1)
  #   expect(@tour.step(1).element.data('bs.popover').tip().filter(':visible').length).toBe 1
  ...
  #   @tour.goTo(1)
  #   @tour.prev()
  #   expect(@tour.step(0).element.data('bs.popover').tip().filter(':visible').length).toBe 1
  ...
  #   @tour.start()
  #   @tour.next()
  #   expect(@tour.current).toBe 0 # tour shows old state until resolving of onShow promise
  #   deferred.resolve()
  #   expect(@tour.current).toBe 1 # tour shows new state after resolving onShow promise
  ...
  #   @tour.start()
  #   @tour.next()
  #   expect(@tour.current).toBe 0 # tour shows old state until resolving of onHide promise
  #   deferred.resolve()
  #   expect(@tour.current).toBe 1 # tour shows new state after resolving onShow promise
  ...
  #   expect(@tour._isRedirect(@tour.options.basePath + @tour.step(0).path, 'test/test.html')).toBe false
  ...
  #   @tour.addStep element: $('<div></div>').appendTo('body')
  #   @tour.start()
  #   expect(test).toBe 0 # tour does not run onNext when next step is not called
  #   @tour.next()
  #   expect(test).toBe 2 # tour runs onNext when next step is called
  ...
  #   @tour.next()
  #   @tour.prev()
  #   expect(test).toBe 2 # tour runs onPrev when prev step is called
  ...
  #     onPrev: -> test = 2
  #   @tour.start()
  #   expect(test).toBe 0 # tour does not run onPrev when prev step is not called
  #   @tour.next()
  #   @tour.prev()
  #   expect(test).toBe 2 # tour runs onPrev when prev step is called
  ...
  #   @tour = new Tour
  #   template = $(@tour.options.template)
  ...
  #   expect($._data($element[0], 'events').click[0].namespace).toBe "tour-#{@tour.options.name}"
  ...
  #   @tour.addStep element: $('<div></div>').appendTo('body')
  #   @tour.showStep(0)
  #   expect($('.popover').hasClass("tour-#{@tour.options.name}")).toBe true
  #   expect($('.popover').hasClass("tour-#{@tour.options.name}-0")).toBe true
  ...
  #   @tour.showStep 0
  #   expect($element.hasClass "tour-#{@tour.options.name}-element").toBe true
  #   expect($element.hasClass "tour-#{@tour.options.name}-0-element").toBe true
  ...
  #   @tour = new Tour
  #   @tour.addStep element: $('<div></div>').appendTo('body')
  #   spyOn(@tour.options.storage, 'setItem').andCallFake ->
